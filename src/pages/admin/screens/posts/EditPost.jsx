...
const EditPost = () => {
  ...
  const handleGenerateWithAI = async () => {
    if (!title) {
      toast.error("Title is required to generate content");
      return;
    }

    setAiLoading(true);
    try {
      const intro = await generateBlogBody({ title, section: "intro" });
      const description = await generateBlogBody({ title, section: "description" });
      const conclusion = await generateBlogBody({ title, section: "conclusion" });
      const fullText = `${intro}\n\n${description}\n\n${conclusion}`;

      if (fullText) {
        setBody(fullText);

        toast.success("Generated blog body");
      } else {
          ...
          <article className="flex-1">
            ...
            <div className="d-form-control w-full">
              ...
              <button
                ...
                disabled={aiLoading}
                className="my-2 bg-blue-500 text-white px-4 py-2 rounded-lg font-semibold disabled:opacity-60"
              >
